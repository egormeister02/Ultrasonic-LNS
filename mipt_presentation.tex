...
{\small МНШ лаб. 69 «Разработка теоретических основ и методов практической реализации ЛНС привязной высотной 
беспилотной платформы»\qquad  Руководитель д.т.н., проф. Вишневский В.М.}
\end{beamercolorbox}%
...
{\large Смирнов Егор Ильич}
 & $\quad$ & \multirow{3}{*}{\includegraphics[height=3.7cm]{images/My_photo.jpg}}  \\
 && \\
...
\vspace{0.3em}
При трилатерации в 3D точность определения координат зависит не только от ошибки измерения расстояний до якорей, но и от их геометрического расположения.\\\vspace{0.3em}
\begin{columns}
  \column{0.5\textwidth}
  \hspace*{1em}
В силу относительно небольшого расстояния между приемниками на платформе, ошибка в плоскости XY будет значительной.\\
  \textbf{Проблема:} при $H \ll a$ (малая высота) ошибка по $z$ резко возрастает.
  \column{0.4\textwidth}
  \centering
...
\begin{frame}{GDOP — зависимость от высоты}
\begin{columns}
  ...
  \vspace{2em}
  \begin{itemize}
    \item При $H \to 0$: GDOP $\to \infty$ (невозможно определить $z$)
    \item Оптимум: $H \approx a$ (высота равна стороне квадрата)
    \item При $H \gg a$: ошибка растёт линейно с высотой
  \end{itemize}
\end{columns}
\end{frame}
...
\begin{frame}{Linear Loss (L1)}
Альтернативная целевая функция:
\[
\boxed{
\; \min_{\mathbf{p}}\; J_{L1}(\mathbf{p}), \qquad
J_{L1}(\mathbf{p}) = \sum_{i=1}^{N} w_i\,\bigl|\,\hat r_i - \|\mathbf{p}-\mathbf{a}_i\|\,\bigr|
}
\]

\begin{itemize}
  \item L1 loss — сумма \textbf{абсолютных} невязок.
  \item Менее чувствительна к \textbf{выбросам}.
  \item Более робастна в условиях импульсного шума.
  \item Использует ту же итерационную процедуру Гаусса-Ньютона.
\end{itemize}
\end{frame}
...
\begin{block}{4 Sensors}
\centering
\footnotesize
\begin{tabular}{lcc}
\toprule
Statistic & analytic err & gn err \\
\midrule
...
\centering 
\begin{itemize}
  \item Экспериментально определить предельную точность измерения расстояния. Определить распределение ошибки.
  \item Получить экспериментальные данные о максимальной дальности детектирования УЗ сигнала. Откалибровать формулу.
  \item Разработать механизм измерения времени распространения УЗ сигнала и механизм синхронизации приемника с источником.
  \item Разработать метод вычислений координат, дающий наименьшую ошибку. При необходимости, разработать механизм учета искажений, вносимых ветром.
  \item Собрать рабочую систему локальной навигации и провести испытания. Собрать данные.
