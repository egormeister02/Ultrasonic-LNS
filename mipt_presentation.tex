...
\begin{columns}
  ...
  \begin{itemize}
    ...
    \item Определить оптимальный метод вычислений координат
  \end{itemize}
...
\begin{table}[h!]
    \centering
    \caption{Аналитический метод, $H=25$м, $n=1000$}
    \begin{tabular}{|c|c|c|}
        \hline
...
\begin{table}[h!]
    \centering
    \caption{Метод Гаусса-Ньютона, $H=25$м, $n=1000$}
    \begin{tabular}{|c|c|c|}
        \hline
        $\sigma$, м & 4 якоря, см & 5 якорей, см \\
        \hline
...
\section{\textbf{Реализация}}

\begin{frame}{Узел БПЛА — передающая часть}
\begin{figure}
\centering
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
...
Аппаратная синхронизация УЗ-пачки и UWB-пакета сигналом EXTTXE ($\Delta t \lesssim 10$\,нс)
\end{frame}

\begin{frame}{Узел якоря — приёмный тракт}
\begin{figure}
\centering
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
...
\vspace{0.5em}
\centering\footnotesize
Полное усиление ${\sim}60$\,дБ; быстрый компаратор LM311P (задержка ${\sim}200$\,нс)
\end{frame}
