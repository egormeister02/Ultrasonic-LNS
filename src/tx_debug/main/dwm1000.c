...
// ОТЛАДОЧНЫЙ передатчик (tx_debug): приём НЕ используется, поэтому RX-параметры (rxPAC, sfdTO)
// не критичны и согласовывать с приёмником не требуется. Преамбула УКОРОЧЕНА до PLEN256 ради
// короткого импульса ~1 мс (PLEN2048 даёт минимум ~2.27 мс — до 1 мс не дойти, см. config.h).
static dwt_config_t s_cfg = {
    .chan           = 5,
    .prf            = DWT_PRF_16M,
    .txPreambLength = DWT_PLEN_256,     // короткая преамбула (~0.25 мс) ради импульса ~1 мс
    .rxPAC          = DWT_PAC16,        // для PLEN256 (RX в tx_debug не используется)
    .txCode         = 4,
    .rxCode         = 4,
    .nsSFD          = 0,                // стандартный IEEE SFD (на 110k = 64 симв)
    .dataRate       = DWT_BR_110K,      // 110 кбит/с
    .phrMode        = DWT_PHRMODE_STD,
    .sfdTO          = 321,              // preamble(256) + 1 + SFD(64) = 321 (RX не используется)
};
